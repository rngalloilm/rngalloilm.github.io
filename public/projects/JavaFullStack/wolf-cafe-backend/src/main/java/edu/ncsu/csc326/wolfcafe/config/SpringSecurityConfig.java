...
public class SpringSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain ( final HttpSecurity http ) throws Exception {
        http.csrf( ( csrf ) -> csrf.disable() ).authorizeHttpRequests( ( authorize ) -> {
            ...
            authorize.requestMatchers( "/api/locations/**" ).permitAll();
            authorize.requestMatchers( "/api/locations" ).permitAll();
            authorize.requestMatchers( "/api/orders/**" ).permitAll();
            authorize.requestMatchers( "/api/menu/**" ).permitAll();
            authorize.requestMatchers( "/api/location/**" ).permitAll();
            authorize.requestMatchers( "/api/document/**" ).permitAll();
            authorize.requestMatchers( "/api/document/pdf/**" ).permitAll();
