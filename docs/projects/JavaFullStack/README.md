...
```
## EXTRA CREDIT
## Multiple inventories (Refactoring)
* We will have multiple locations for our workers. This allows for an easy expansion of the wolf-cafe business, since now, to add a new location, all they need to do is navigate to the locations page, and add the new information. This makes it easy to categorize both inventories, and staff members based on they physical location at which they work. They will still be in the system, and can easily be transferred to another location if that is required.

## End of day order deletion (Functionality)
* At the end of each day, based on location, all unfulfilled and un-picked-up orders will be wiped from the system. This means that their content will be deleted from our backend, and will not have any analytics run on them. THis ensures that all of our analysis will have been done on real orders and not any attempted DoS orders or any outliers.

## Automated Frontend Testing Automated Frontend (wolf-cafe-frontend/src/_test_) 
* do 'npm install react-tooltip && npm install --save-dev jest babel-jest @babel/preset-env @babel/preset-react react-test-renderer && npm i jest-environment-jsdom --save-dev && npm install --save-dev @babel/preset-react && npm test'
* Can also see tests running in github actions (Automated Frontend action)\

## Security Audit 
* Findings are listed [here](https://github.ncsu.edu/engr-csc326-fall2024/csc326-TP-204-1/wiki/Security-Audit)

## Anonymous Orders
* You can order and pick up orders anonymously by navigating to the respective guest pages off the login screen
* A staff member must fulfill your order before you can pick it up still

## Order History 
* Fulfilled orders can be seen in the order history page, which is available to all **_STAFF_** members
* There is also a graph there allowing you to see previous order history by day for the given location
* There is also a computed list of the top 5 ordered items/recipes displayed
