...
function SeedTab(props) {
        ...
        .then(async (res) => {
          for (let seed in res.data) {
            let obj = {
              ...
              isCheckedOut: res.data[seed].isCheckedOut,
              checkedOutBy: res.data[seed].checkedOutBy
            };
  ...
  return (
    <div>
      {data ? (
        <TableComponent propagateLink="/propagate/seed-material" editLink= "/material/seeds/edit" addLink="/material/seeds/add" status={"active"} material={"seed"} rows={rows} columns={columns} loading={loading1} error={error} user={props.user} isOfflineSelection={true}/>
      ) : (
        <p></p>
      )}
    </div>
