...
function RametTab(props) {
        ...
        .then(async (res) => { 
          for (let ramet in res.data) {
            let obj = {
              ...
              active: res.data[ramet].active,
              isCheckedOut: res.data[ramet].isCheckedOut,
              checkedOutBy: res.data[ramet].checkedOutBy
            }
  ...
  return (
    <div>
      {data ? (
        <TableComponent editLink= "/material/ramet-material/edit" addLink="/material/ramet-material/add" status={"active"} material={"ramet"} rows={rows} columns={columns} loading={loading} error={error} user={props.user} isOfflineSelection={true}/>
      ) : (
        <p></p>
      )}
    </div>
