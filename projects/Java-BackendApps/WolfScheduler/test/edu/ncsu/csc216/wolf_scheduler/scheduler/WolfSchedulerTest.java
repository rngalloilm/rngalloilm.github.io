...
public class WolfSchedulerTest {
	...
	public void testAddCourseToSchedule() {
		...
		//Add two courses that conflict
		//Period: 935 to 1025 MWF
		ws.addCourseToSchedule("CSC 226", "001");
		try {
			//Period: 0910 to 1100 MW
	...
	public void testAddEventToSchedule() {
		...
		//Add two events that conflict
		//Period: 900 to 1100 M
		ws.addEventToSchedule("ConflictOne", "M", 1000, 1200, "Rock");
		try {
			//Period: 1000 to 1200 M
			ws.addEventToSchedule("ConflictTwo", "M", 1100, 1300, "Roll");
		} catch (IllegalArgumentException e) {
	...
	/**
	 * Test WolfScheduler with conflicting Event and Course.
	 */
	@Test
	public void testAddActivitiesToScheduleWithConflict() {
		WolfScheduler ws = new WolfScheduler(validTestFile);
		
		//Add one event and one course that conflict
		//Event period: 900 to 1100 M
		ws.addEventToSchedule("ConflictOne", "M", 900, 1100, "Rock");
		assertEquals(1, ws.getScheduledActivities().length);
		assertEquals(1, ws.getFullScheduledActivities().length);
		try {
			//Course period: 0910 to 1100 MW
			ws.addCourseToSchedule("CSC 116", "001");
		} catch (IllegalArgumentException e) {
			assertEquals("The course cannot be added due to a conflict.", e.getMessage());
		}
	...
	public void testGetScheduledActivities() {
		...
		assertTrue(ws.addCourseToSchedule(NAME, SECTION));
		assertTrue(ws.addCourseToSchedule("CSC 226", "001"));
		ws.addEventToSchedule("Lunch", "MWF", 1400, 1500, "Food");
		assertTrue(ws.addCourseToSchedule("CSC 116", "002"));
		...
		assertEquals("MWF 2:00PM-3:00PM", schedule[2][3]);
		//Row 4
	...
	public void testGetFullScheduledActivities() {
		...
		assertEquals("MWF 2:00PM-3:00PM", schedule[2][5]);
